...
# file = pd.read_excel("./data/ex8data1.xlsx", sheet_name="X", header=None)
...
#         variance_square = (
#             variance_denominator / mean
#         )  # It is important to note it is variance

#         variance_diag = np.diag(variance_square)  # diagonal values of the variance
#         variance_sq_determinant = np.linalg.det(
#             variance_diag
...
#     def calc_tf_fp_fn(
#         self, prob_x, epsilon, file_cv
#     ):  # This is a function to find true positives, false positives and false negatives
...
#         # values of the list
#         truePositive, falsePositive, falseNegative = self.calc_tf_fp_fn(
#             prob_x, epsilon, file_cv
#         )
...
#         epsilon_list = [
#             item for item in prob_cv if item <= prob_cv.mean()
#         ]  # list of items with value probability
...
#         anomaly_List = (
#             []
#         )  # This list will contain 0 or 1 value and that will tell if an item is anomalous or not
